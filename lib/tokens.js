...
async function resolveToken(client, Account, address, registryEntry) {
	...
	/*
	 * The on-chain ticker is used as published.
	 *
	 * An earlier version renamed Keeta's tokenized fiat (USD, EUR, JPY...) to
	 * KEETAUSD and similar, on the theory that tax software would otherwise
	 * resolve them to sovereign fiat and misprice them. That was reversed.
	 *
	 * Those tokens are issued through Keeta Personal, a regulated
	 * money-movement product: holders are KYC verified, balances arrive by
	 * direct deposit and leave by ACH, and accounts carry real bank details.
	 * They are denominations of currency, not tokens that happen to track one,
	 * and they redeem 1:1. Tax software computes no gain or loss on fiat, which
	 * is the correct treatment for cash.
	 *
	 * Renaming them would have demanded a manually entered price for every row
	 * of a balance that is one dollar by construction. That is a worse outcome
	 * than the mispricing it was meant to prevent.
	 */
	return ({
		address,
