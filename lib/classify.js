 ...
 * ============================================================================
 */
function extractAnchorPayloads(voteStaple) {
	const out = [];
	if (!voteStaple) {
		return (out);
	}
	...
	for (const block of (json && json.blocks) || []) {
		for (const op of block.operations || []) {
			...
			const d = decodeExternal(op.external);
			if (d) {
				out.push(d);
			}
		}
...
function classifyStaple(entry, ctx) {
	...
	/* Anchor instructions for this staple, decoded once and shared by every
	 * row it produces. */
	const payloads = extractAnchorPayloads(entry.voteStaple);
...
const API = {
	...
	extractAnchorPayloads, attributeBridge,
