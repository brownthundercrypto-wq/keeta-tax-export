...
const _anchorLib = (typeof require === 'function') ? require('./anchors') : globalThis.KeetaTax;
const decodeExternal = _anchorLib.decodeExternal;
const PAYLOAD_FORM = _anchorLib.PAYLOAD_FORM;
...
const REASONS = {
	...
	BRIDGE_IN: 'bridge transfer in, confirmed from the anchor instruction',
	BRIDGE_OUT: 'bridge transfer out to another chain',
	FIAT_OFFRAMP: 'sent to a fiat rail, which cannot be matched on any chain',
	ANCHOR_PAYLOAD_UNREADABLE: 'an anchor instruction could not be decoded',
	DECLARED_ANCHOR: 'the recipient describes itself as an anchor, so this may not be an ordinary payment',
	ANCHOR_UNCHECKED: 'a recipient could not be checked against bridge detection',
...
/*
 * ============================================================================
 * BRIDGE ATTRIBUTION: TWO INDEPENDENT SIGNALS, EITHER SUFFICIENT
 * ============================================================================
 *
 * A bridge transfer announces itself two ways, and THEY LEGITIMATELY DIFFER:
 *
 *   PAYLOAD      an `external` anchor instruction on some operation in the
 *                staple. Its JSON names the SERVICE anchor
 *                (`keeta_aabuj72z...`), which has zero observed counterparty
 *                traffic because it never holds funds.
 *   COUNTERPARTY the account the money actually moved to or from, which is the
 *                custody anchor (`EVM_ANCHOR`).
 *
 * Requiring both would miss every transfer where only one is present.
 * Requiring a specific one would silently pick a side. So EITHER is sufficient,
 * and which one fired is RECORDED on the flag: if the two turn out to diverge
 * more than expected, that record is the evidence.
 * ============================================================================
 */
/*
 * Raw `external` strings for a staple. Decoding is deliberately NOT done here:
 * form A needs DecompressionStream, which is async, and classification must
 * stay synchronous so the CLI and the page produce identical output. The decode
 * happens once per staple in pipeline.prefetchPayloads.
 */
function extractExternals(voteStaple) {
	const out = [];
	if (!voteStaple) {
		return (out);
	}
	...
	for (const block of (json && json.blocks) || []) {
		for (const op of block.operations || []) {
			if (typeof op.external === 'string' && op.external.length > 0) {
				out.push(op.external);
			}
		}
	}
	return (out);
...
/*
 * Decide whether this movement is a bridge leg, and on what evidence.
 * Returns null when neither signal fires.
 */
function attributeBridge(movement, direction, payloads, bridgeAnchors) {
	/*
	 * REGISTRY anchors only. A merely self-declared anchor keeps its own,
	 * weaker DECLARED_ANCHOR flag below: one real mainnet example is a KYC
	 * anchor, and calling that a bridge would invite the user to mark a genuine
	 * disposal as a non-taxable transfer.
	 */
	const counterpartyHits = [...movement.counterparties].filter((c) => bridgeAnchors.has(c));

	/* A payload counts only if it names an anchor we know, or is a destination
	 * form (which is self-evidently an anchor instruction). */
	const relevant = payloads.filter((p) =>
		p.form === PAYLOAD_FORM.CRYPTO_DEST ||
		p.form === PAYLOAD_FORM.FIAT_REF ||
		(p.form === PAYLOAD_FORM.ANCHOR_JSON && (p.anchors || []).some((a) => bridgeAnchors.has(a))));

	if (counterpartyHits.length === 0 && relevant.length === 0) {
		return (null);
	}

	const signals = [];
	if (relevant.length > 0) { signals.push('payload'); }
	if (counterpartyHits.length > 0) { signals.push('counterparty'); }

	const dest = relevant.find((p) => p.form === PAYLOAD_FORM.CRYPTO_DEST) || null;
	const fiat = relevant.find((p) => p.form === PAYLOAD_FORM.FIAT_REF) || null;

	let reason;
	if (fiat && direction === 'out') {
		reason = REASONS.FIAT_OFFRAMP;
	} else if (direction === 'in') {
		reason = REASONS.BRIDGE_IN;
	} else if (dest) {
		reason = REASONS.BRIDGE_OUT;
	} else {
		/* Anchor involvement without a destination we can name. The weaker,
		 * pre-existing signal. */
	...
	return ({
		reason: reason,
		/* Which signal fired. 'payload' or 'counterparty' alone means the two
		 * disagreed on this row; both means they corroborated. */
		signals: signals,
		agreed: signals.length === 2,
		chainId: dest ? dest.chainId : null,
		destination: dest ? dest.address : null,
		orderRef: fiat ? fiat.orderRef : null,
		anchors: counterpartyHits.map((a) => ({
			address: a,
			name: bridgeAnchors.has(a)
				? bridgeAnchors.get(a).name
				: ((anchors.get(a) && (anchors.get(a).name || anchors.get(a).description)) || 'self-declared anchor'),
			source: bridgeAnchors.has(a) ? 'registry' : 'on-chain'
		}))
	});
}
...
function buildTransferResult(leg, args) {
	const { stapleHash, timestamp, effects, bridgeAnchors, anchors, payloads, fee } = args;
	 ...
	 * that a send was not a bridge. The row is still emitted; it is just never
	 * emitted as a clean withdrawal.
	 *
	 * Two sources, because the registry alone missed 98 real bridge transfers:
	 *   - the shipped registry, matched in BOTH directions (free), and
	 *   - anchors that self-declare on-chain, matched on OUTGOING legs only.
	 * See lib/anchors.js for why the registry could not be the ceiling.
	 */
	const discovered = [...movement.counterpartiesOut].filter((c) =>
		!bridgeAnchors.has(c) && anchors && anchors.get(c) && anchors.get(c).isAnchor === true);

	/*
	 * Registry anchors and anchor instructions, attributed together so that
	 * either signal is sufficient and whichever fired is recorded.
	 */
	const bridge = attributeBridge(movement, isIncoming ? 'in' : 'out', payloads || [], bridgeAnchors);
	if (bridge) {
		flags.push(bridge);
	}

	/*
	 * An anchor instruction we could not read is SURFACED, never ignored. A
	 * failed decode has twice been mistaken for absent data on this project;
	 * treating it as "not a bridge" would repeat that in the one place it
	 * costs the user money.
	 */
	const unreadable = (payloads || []).filter((p) => p.form === PAYLOAD_FORM.UNKNOWN);
	if (unreadable.length > 0) {
		flags.push({
			reason: REASONS.ANCHOR_PAYLOAD_UNREADABLE,
			count: unreadable.length,
			samples: unreadable.slice(0, 2).map((p) => ({ length: p.length, sample: p.sample }))
		});
	}

	/*
	 * Discovered anchors are a WEAKER claim and must not borrow the bridge
	 * wording. All we know is that the account calls itself an anchor, and
	 * anchors do several unrelated jobs: bridging, KYC, storage, issuance. A
	 * real example is "KYC Test Anchor Root", which is not a bridge at all.
	 *
	 * Telling someone a KYC check was a possible bridge transfer invites them to
	 * mark a genuine disposal as a non-taxable self-transfer, which is a worse
	 * error than not flagging it. So we report the account's own words and ask
	 * what the transaction was, rather than asserting what it was.
	 */
	if (discovered.length > 0) {
		flags.push({
			reason: REASONS.DECLARED_ANCHOR,
			anchors: discovered.map((a) => ({
				address: a,
				name: anchors.get(a).name || null,
				description: anchors.get(a).description || null,
				declared: anchors.get(a).name || anchors.get(a).description || 'an anchor',
				source: 'on-chain'
			}))
		});
	}
	...
	/*
	 * "We could not check this recipient" must never render as "not a bridge".
	 * Surfaced rather than swallowed, per the fail-loudly rule.
	 */
	const unchecked = [...movement.counterpartiesOut].filter((c) =>
		!bridgeAnchors.has(c) && anchors && anchors.get(c) && anchors.get(c).checked === false);
	if (unchecked.length > 0) {
		flags.push({
			reason: REASONS.ANCHOR_UNCHECKED,
			count: unchecked.length,
			statuses: [...new Set(unchecked.map((c) => anchors.get(c).status))]
		});
	}
...
function classifyStaple(entry, ctx) {
	const { ourKey, bridgeAnchors, tokens, anchors } = ctx;
	...
	/* Anchor instructions, decoded once per staple in the prefetch step and
	 * shared by every row this staple produces. */
	const payloads = (ctx.payloads && ctx.payloads.get) ? (ctx.payloads.get(stapleHash) || []) : [];
...
const API = {
	...
	extractExternals, attributeBridge,
