 ...
 *
 * Operations are still consulted, but ONLY for context that effects genuinely
 * do not carry: the `external` memo field on SEND. Never for amount, direction,
 * or classification.
 *
 ...
 *   - Decimals derive from the network ID. KTA is 18 on mainnet, 9 on testnet.
...
const REASONS = {
	MULTI_TOKEN: 'multi-token staple (swap/trade), out of scope for KTA-only v1',
	UNKNOWN_TOKEN: 'non-KTA token, decimals not resolvable, refusing to guess',
...
/*
 * Classify a single staple into either a CSV row or a flag.
 *
 * Returns one of:
...
function classifyStaple(entry, ctx) {
	const { ourKey, baseToken, baseTokenSymbol, baseTokenDecimals, bridgeAnchors, tokenRegistry } = ctx;
	...
	const nets = netDeltasByToken(ourEffect.fields.balance);
	...
	/*
	 * More than one token moved on net => this is a trade/swap. KTA-only v1
	 * does not price or pair these. Detect, exclude, report. Do NOT emit the
	 * legs as separate transfers -- that would double-count a single trade.
	 */
	if (moved.length > 1) {
		return ({
			kind: 'flag',
			reason: REASONS.MULTI_TOKEN,
			detail: {
				stapleHash: stapleHash,
				timestamp: timestamp,
				legs: moved.map(([token, v]) => ({
					token: token,
					symbol: tokenRegistry.get(token) ? tokenRegistry.get(token).symbol : null,
					net: v.net.toString(),
	...
	const [token, movement] = moved[0];

	/*
	 * Single token, but not the base token. We have no on-chain decimals for
	 * any non-KTA token. Decimals ARE readable on-chain (info.metadata carries
	 * decimalPlaces as base64 JSON), so this is no longer a divisor problem: it
	 * is that only KTA has been verified end to end, and several Keeta tokens
	 * use tickers that collide with real currencies. Hard fail this row.
	 */
	if (token !== baseToken) {
		const known = tokenRegistry.get(token);
		return ({
			kind: 'flag',
			...
			detail: {
				...
				registryDecimals: known && known.decimals !== undefined ? known.decimals : null,
				net: movement.net.toString(),
				direction: movement.net > 0n ? 'in' : 'out'
			}
		});
	}
	...
	if (movement.grossIn > 0n && movement.grossOut > 0n) {
		...
		const dustLimit = parseUnits(DUST_THRESHOLD_KTA, baseTokenDecimals);
		...
		flags.push({
			...
			hiddenLeg: hiddenLeg,
			/*
			 * Suppressed from the per-row listing because it cannot be
			 * represented in the report at all -- but still counted and
			 * reported in aggregate. See DUST_THRESHOLD_KTA.
			 */
	...
	return ({
		...
		row: {
			...
			feeUnits: effects.metadata && typeof effects.metadata.feeUnits === 'bigint'
				? effects.metadata.feeUnits
				: null,
			memo: null
		},
		flags: flags
...
const API = {
	...
	REASONS, YEAR_BOUNDARY_WINDOW_MS, DUST_THRESHOLD_KTA
};
