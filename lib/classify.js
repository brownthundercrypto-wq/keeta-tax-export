...
function buildTransferResult(leg, args) {
	...
	if (registryHits.length > 0 || discovered.length > 0) {
		flags.push({
			reason: REASONS.POSSIBLE_BRIDGE,
			anchors: registryHits
				.map((a) => ({ address: a, name: bridgeAnchors.get(a).name, source: 'registry' }))
				/*
				 * Show whatever the account calls itself. Not every self-declared
				 * anchor is a bridge -- a KYC anchor matches too -- so the user
				 * needs the actual wording to judge, not a generic label.
				 */
				.concat(discovered.map((a) => ({
					address: a,
					name: anchors.get(a).name || anchors.get(a).description || 'self-declared anchor',
					source: 'on-chain'
				})))
		});
	}
