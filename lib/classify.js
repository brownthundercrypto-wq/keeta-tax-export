...
function classifyStaple(entry, ctx) {
	 ...
	 * populated), fee columns blank because routing fees are already netted
	 * into these figures by the per-token netting above.
	 */
	if (moved.length > 1) {
		if (outLegs.length !== 1 || inLegs.length !== 1) {
			/*
			 * Anything else is not a trade. The one real example observed is a
			 * token launch: CREATE_IDENTIFIER + SET_INFO + TOKEN_ADMIN_SUPPLY
			 * with three tokens received and none sent. Flag rather than invent
			 * a shape for it.
			 */
			return ({
				kind: 'flag',
		...
		return ({
			...
			row: {
				...
				feeUnits: effects.metadata && typeof effects.metadata.feeUnits === 'bigint' ? effects.metadata.feeUnits : null,
				memo: null
			},
			flags: tradeFlags
		});
	...
	const only = legs[0];
	const token = only.token;
	const movement = only.movement;
	const tokenInfo = only.info;
