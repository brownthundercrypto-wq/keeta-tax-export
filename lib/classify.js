...
function buildTransferResult(leg, args) {
	const { stapleHash, timestamp, effects, bridgeAnchors, anchors, fee } = args;
	 ...
	 */
	const registryHits = [...movement.counterparties].filter((c) => bridgeAnchors.has(c));
	...
	/*
	 * Registry hits are anchors we have identified as BRIDGES, so naming the
	 * behaviour is fair.
	 */
	if (registryHits.length > 0) {
		flags.push({
			reason: REASONS.POSSIBLE_BRIDGE,
			anchors: registryHits.map((a) => ({
				address: a, name: bridgeAnchors.get(a).name, source: 'registry'
			}))
		});
	}
