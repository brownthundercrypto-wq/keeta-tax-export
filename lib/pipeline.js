...
function renderFlaggedMd(ctx, groups, stats, generatedAtIso) {
	 ...
	 */
	const ordered = reasons.slice().sort((a, b) => {
		const ea = isExclusion(a) ? 0 : 1, eb = isExclusion(b) ? 0 : 1;
	...
	for (const reason of ordered) {
		...
		if (!isExclusion(reason) && !wroteReviewHeading) {
			wroteReviewHeading = true;
			L.push('# In your file, but check these');
			L.push('');
			L.push('These rows **are in your CSV**. They are flagged because there is a judgement only you can make: whether an address is yours, whether a send was a sale, which tax year something belongs to.');
			L.push('');
		}
...
const API = {
	...
	groupFlags, renderFlaggedMd, explainReason, describeFlag, FLAG_ORDER, isExclusion, EXCLUDED_REASONS
};
