...
function buildCsv(rows) {
	...
	/*
	 * EVERY row must carry an asset in Sent or Received.
	 *
	 * A row with both blank -- even with a valid Fee Currency and Fee Amount --
	 * makes CoinLedger reject the ENTIRE FILE, not just that row **[DATA]**:
	 *
	 *   Unexpected transaction type "OutgoingCurrency: NULL | OutgoingAmount:
	 *   NULL | IncomingCurrency: NULL | IncomingAmount: NULL" on item 2.
	 *
	 *   0 transactions imported, 1 account failed
	 *
	 * A known-good row earlier in the same file did not import either. So one
	 * fee-only transaction anywhere in an export would destroy the whole import
	 * for that user, with an error naming an internal field they have never seen.
	 *
	 * Fee-only staples are excluded upstream for this reason. This assertion
	 * exists so a later change cannot quietly reintroduce them: the failure is
	 * total and invisible until the user tries to import.
	 */
	const csvRows = rows.map(toCsvRow);
	for (let i = 0; i < csvRows.length; i++) {
		const f = csvRows[i].split(',');
		/* Fields 2..5 are Asset Sent, Amount Sent, Asset Received, Amount
		 * Received. Unquoted and never contain commas: symbols and decimal
		 * strings only. */
		const hasSent = f[2] !== '' && f[3] !== '';
		const hasReceived = f[4] !== '' && f[5] !== '';
		if (!hasSent && !hasReceived) {
			throw new Error(
				`Row ${i + 1} has no asset in Sent or Received. CoinLedger rejects the ` +
				`entire file for this, not just the row. Refusing to write.\n  ${csvRows[i]}`
			);
		}
	}

	/* CRLF line endings (RFC 4180). Caller writes as UTF-8 without BOM. */
	const out = [headerLine, ...csvRows].join('\r\n') + '\r\n';

	/*
	 * A CSV downloaded on iOS was reported with every line ending \r\r\n. This
	 * code cannot produce that: the join above is the only place line endings
	 * are written, and the Blob is constructed from this string untouched. The
	 * doubling therefore happened after download, in whatever moved the file.
	 *
	 * Asserted anyway, because a stray \r would be invisible in review and
	 * CoinLedger parses on exact bytes. Same reasoning as the header check.
	 */
	if (out.indexOf('\r\r') !== -1 || /(^|[^\r])\n/.test(out)) {
		throw new Error('CSV line endings are not clean CRLF. Refusing to write.');
	}
...
function explainReason(reason) {
	switch (reason) {
		...
		case REASONS.FEE_ONLY:
			return ('**These are real KTA disposals with no transfer to attach them to.** The only thing that moved was the network fee. Nothing was sent and nothing was received.\n\nA CoinLedger row needs an asset and an amount in either the Sent or the Received columns, and these transactions have neither, so there is nowhere in the file to put them. That is the whole reason they are missing. It is not a judgement about them.\n\n**You paid the fee, so you disposed of that KTA.** If the total matters to your return, add it by hand as a disposal of the amount shown.\n\nThis usually happens when you publish a transaction on behalf of another account and pay its fee.');
...
/*
 * Two kinds of flag, and conflating them was misleading.
 *
 * EXCLUDED: the transaction is NOT in the CSV, for a mechanical reason. There is
 * no judgement for the user to make and nothing they can decide differently.
 * Fee-only staples are the clearest case: they are real KTA disposals, but a
 * CoinLedger row needs an asset and amount in Sent or Received and these have
 * neither, so there is nowhere to put them.
 *
 * REVIEW: the row IS in the CSV, and the user has a real judgement to make --
 * was that send a sale, is that address yours, which tax year does this fall in.
 *
 * Filing both under "needs your review" told people they had a decision to make
 * about rows they cannot affect, and buried the ones where they genuinely do.
 */
...
function isExclusion(reason) {
	return (EXCLUDED_REASONS.has(reason));
}

/*
 * Within the rows that ARE in the file, confidence differs enormously and
 * listing them together flattened that.
 *
 * CONFIRMED means the transaction itself told us what it was: the anchor
 * instruction names the destination chain and address, or names a fiat rail.
 * That is information, not a question. A "possible bridge" is the opposite --
 * we saw an anchor counterparty and no instruction, so we are guessing.
 *
 * Presenting a verified arrival next to a guess invited the user to spend equal
 * attention on both, which is exactly backwards.
 */
const CONFIRMED_REASONS = new Set([
	REASONS.BRIDGE_IN,
	REASONS.BRIDGE_OUT,
...
function isConfirmed(reason) {
	return (CONFIRMED_REASONS.has(reason));
}
...
function renderFlaggedMd(ctx, groups, stats, generatedAtIso) {
	...
	L.push('# Read this before you file');
	L.push('');
	L.push('This file covers three things: transactions that are **not** in your CSV and why, what the tool could establish for certain, and the rows where a judgement is left to you.');
	L.push('');
	...
	/*
	 * Excluded first. A transaction missing from the file is more urgent than a
	 * row that is present and wants a second look.
	 */
	const bucket = (r) => (isExclusion(r) ? 0 : (isConfirmed(r) ? 1 : 2));
	const ordered = reasons.slice().sort((a, b) => (bucket(a) - bucket(b)));

	let wroteExcludedHeading = false;
	let wroteConfirmedHeading = false;
	let wroteReviewHeading = false;

	for (const reason of ordered) {
		const items = groups.get(reason).slice();

		if (isExclusion(reason) && !wroteExcludedHeading) {
			wroteExcludedHeading = true;
			L.push('# Not included in your file');
			L.push('');
			L.push('These transactions are **not in your CSV**. There is no decision for you to make about them: each one is missing for a mechanical reason, explained below. They are listed so that nothing disappears without being accounted for.');
			L.push('');
			L.push('Some of them are still real taxable events. Where that is the case it says so, and you will need to enter them by hand if they matter to your return.');
			L.push('');
		}

		if (isConfirmed(reason) && !wroteConfirmedHeading) {
			wroteConfirmedHeading = true;
			L.push('# What we could confirm');
			L.push('');
			L.push('These rows **are in your CSV**, and the transaction itself told us what it was. The transfer instruction inside it names the destination chain and address, or names a fiat rail. This is not a guess we are asking you to check.');
			L.push('');
			L.push('It is listed because knowing where something went can still change how you treat it. If a destination address is your own wallet, that transfer is not a sale.');
			L.push('');
		}

		if (!isExclusion(reason) && !isConfirmed(reason) && !wroteReviewHeading) {
			wroteReviewHeading = true;
			L.push('# Needs your judgement');
			L.push('');
			L.push('These rows **are in your CSV** and we could not establish what they were. Either the transaction carried no instruction, or it did and we could not read it, or the answer depends on something only you know: whether an address is yours, whether a send was a sale, which tax year something belongs to.');
			L.push('');
		}
...
const API = {
	...
	groupFlags, renderFlaggedMd, explainReason, describeFlag, FLAG_ORDER, isExclusion, EXCLUDED_REASONS, isConfirmed, CONFIRMED_REASONS
};
