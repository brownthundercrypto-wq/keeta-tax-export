...
function toCsvRow(row) {
	 ...
	 *
	 * Fee Currency / Fee Amount are intentionally left BLANK. For transfers the
	 * staple-level `feeUnits` denomination is unverified. For trades the routing
	 * fees are already netted into the sent/received figures by the per-token
	 * netting, which is exactly the case CoinLedger says to leave blank.
	 */
	...
	return (csvRow([
		...
		'', '', type, description, row.stapleHash
	]));
}
		...
		if (result.kind === 'row') {
			...
			for (const f of result.flags) {
				if (f.reason === REASONS.UNPRICEABLE) {
					stats.unpriceableSymbols.add(f.symbol || f.token);
				}
			}
			if (result.flags.some((f) => f.reason === REASONS.UNPRICEABLE)) {
...
function renderFlaggedMd(ctx, groups, stats, generatedAtIso) {
	...
	if (stats.unpriceableRows > 0) {
		...
		L.push('## ⚠️ ' + stats.unpriceableRows + ' rows use tokens CoinLedger cannot price');
		L.push('');
		L.push('Tokens affected: **' + syms + '**');
		L.push('');
		L.push('CoinLedger only has price history for assets it already knows about. For anything else you have to add it as a **custom asset**, and then **enter the price yourself for every transaction, at every date**. There is no automatic pricing and no bulk lookup that fills it in for you.');
		L.push('');
		L.push('**' + stats.unpriceableRows + ' rows means ' + stats.unpriceableRows + ' prices to research and type in.** Decide whether that is worth it before you import. If it is not, you may prefer to import only the assets CoinLedger prices and handle these separately.');
		L.push('');
	}
