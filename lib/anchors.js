...
async function inflateToJson(bytes) {
	for (let i = 0; i < Math.min(bytes.length, 64); i++) {
		...
		try {
			return (JSON.parse(await inflate(bytes.subarray(i))));
		} catch (err) {
			/* A missing DecompressionStream is a runtime fault, not a bad
			 * candidate offset. Surface it instead of scanning past it. */
			if (String(err && err.message).indexOf('DecompressionStream') !== -1) {
				throw err;
			}
		}
	}
...
async function decodeExternal(raw) {
	...
	/* Form A: base64 DER + zlib JSON. `atob` is standard in Node 16+ too, so
	 * this path does not depend on Buffer and runs the same in both. */
	if (!hex) {
		let bytes = null;
		try {
			const bin = atob(raw);
